...
ignoreList = ("gtk3-icon-browser","evince-previewer", "Ted",  "wingide3.2", "python3.4", "feh","xfce4-power-manager-settings", "picom","compton","yad-icon-browser", "avahi-discover", "bssh", "bvnc", "qv4l2", "foot", "footclient", "foot-server", "qvidcap" )
	...
	footer_items = [
		{
			...
			"action": "Execute", 
			"cmd": "ghostty", 
			"icons": ["com.mitchellh.ghostty", "terminal", "x-terminal-emulator", "org.gnome.Terminal"]
		},
		{
			"label": "Reconfigure", 
			"action": "Reconfigure", 
		...
		},
		{
			"label": "Background", 
			"action": "Execute", 
			"cmd": f"{userhome}/.config/rofi/wallselect/wallselect.sh",
	...
	for item in footer_items:
		...
		if is_static:
			...
			if iconPath:
				out += f' icon="{iconPath}"'
			if item["cmd"]:
				out += f'><action name="{item["action"]}"><command><![CDATA[{item["cmd"]}]]></command></action></item>'
			else:
				out += f'><action name="{item["action"]}" /></item>'
			print(out)
